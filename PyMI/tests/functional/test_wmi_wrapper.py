...
IMPERSONATION_LEVELS = [None, "identify", "impersonate"]
...
class TestBasicConnections(unittest.TestCase):
    ...
    # NOTE(cpurcaru): We don't support authentication, authority or privileges
    ...
    # NOTE(cpurcaru): We don't support 'namespace' as a separate arg
    # it is part of 'moniker' arg
...
class TestThreadedConnection(unittest.TestCase):
    ...
    # NOTE(cpurcaru): This should not fail in this case
...
class TestMoniker(unittest.TestCase):
    ...
    def test_moniker(self):
        ...
        for computer in COMPUTERS:
            ...
            for impersonation_level in IMPERSONATION_LEVELS:
                for namespace in NAMESPACES:
                    moniker = wmi._construct_moniker(
                        computer=computer,
    ...
    def test_moniker_root_namespace(self):
        ...
        self.assertEquals(wmi._construct_moniker(namespace="default"), "winmgmts:root/default")
        self.assertEquals(wmi._construct_moniker(namespace="root/default"), "winmgmts:root/default")
...
class TestFunctions(unittest.TestCase):
    ...
    def test_unsigned_to_signed(self):
        tests = [
           (0, 0),
           (-1, 0xffffffff),
           (+1, 1),
        ...
        for signed, unsigned in tests:
            self.assertEquals(wmi.unsigned_to_signed(unsigned), signed)
    ...
    # NOTE(cpurcaru): this is not needed by PyMI, being just a convenience
    # function.
...
class TestWMI(unittest.TestCase):
    ...
    def setUp(self):
        # We do not support the 'namespace' argument, we expect the full moniker path
        self.connection = wmi.WMI(moniker="root/cimv2")
        self.logical_disks = set(self.connection.Win32_LogicalDisk())

class TestNamespace(TestWMI):

    # NOTE(cpurcaru): We don't support subclasses feature
    ...
    # NOTE: unsupported arg: get_classes
    ...
    def test_watch_for(self):
        ...
        self.assert_(isinstance(watcher, wmi._EventWatcher))
...
class TestClass(TestWMI):
    ...
    #     wmi_class = GetObject("winmgmts:Win32_ComputerSystem")
    #     self.assert_(wmi._wmi_class(None, wmi_class)._namespace)

    # NOTE(cprucaru): We don't support query through class
    # def test_query(self):
    #     self.assertEquals(
    ...
    def test_new(self):
        # (ToDo) : This should be modified to compare the
        # class itself not the class name
        process = self.connection.Win32_Process.new()
        proc_cls = process._instance.get_class_name()
        self.assertEquals(proc_cls.lower(),
                          self.connection.Win32_process.class_name.lower())
...
class TestWatcher(TestWMI):
    ...
    def test_creation(self):
        ...
        self.assert_(isinstance(found_disk, wmi._Instance))
...
class TestMethods(TestWMI):
    ...
    # NOTE(cpurcaru): We don't support showWindow argument
...
class TestInstances(TestWMI):
    ...
    # NOTE(cpurcaru): instance eq not supported
    ...
    # NOTE(cpurcaru): references feature not supported
    ...
    # NOTE(cpurcaru): derication feature not supported
